...
(* AB: In Deutsch.thy we introduced the abbreviations zero and one and then used |zero\<rangle>
and |one\<rangle>. Is there a good reason to change across theories ? *)
...
(* AB: Above it's not a good title for a subsection. The reader has no idea what j is *)

(* Gives back the part of j starting at position s being t qubits long. 
E.g. j=$|01011\rangle$, s=2 and l=3 gives [1,0,1] *)

(* AB: The comment above is unclear. The sentence has no subject. Is the subject "to_list_bound" ?
If so, why is j above a state while it's a natural number below ? Why is the output above a list
of natural numbers while it's a list of matrices below. The comment does not help, it's even
confusing. 
Also, it seems that you start counting from 1 for the position s while all the rest of the library
takes the convention that we start counting from 0. *)
...
Takes a list and the number of elements in this list and returns the tensor product of the elements *)

(* AB: Again, is the subject of the sentence "Product representation" or "pow_tensor_list" ? *)
...
(* Defines the part of j where s is the start position and t the number of bits as tensor product of 
single qubits where j is a decimal number that is smaller than m. E.g. $|j_1,...,j_n\rangle$ and s=2 
and l=3 gives $|j_2,j_3,j_4\rangle$ *)
...
lemma to_list_bound_one [simp]: (* Maybe "to_list_bound_length_1", it's a bit longer but clearer. *)
...
proof(rule Nat.nat_induct_at_least[of "n-s" t])
  show "t \<ge> n - s" using assms(4) by simp
next
...
proof
  ...
  have "(j\<Otimes> n 1 m j) = (if (bin_rep m j)!(n-1) = 0 then zero else one)"  using to_tensor_prod_def by simp 
...
next
  ...
  then show "(j\<Otimes> n 1 m j) = zero" using to_tensor_prod_def by simp 
qed
...
proof-
  ...
  moreover have "\<dots> = (m mod 2) div 2^0" using assms(2) by simp
  finally have "\<dots> = 0" using assms(1) by simp
...
proof
  ...
  next
    ...
    show "( |unit_vec (2^k) m\<rangle> ) $$ (i,j) = ( |unit_vec (2^(k-1)) (m div 2)\<rangle> \<Otimes> zero) $$ (i,j)"
    proof(rule disjE)
      show "i div 2 = m div 2 \<or> i div 2 \<noteq> m div 2" by simp
    next
...
proof-
  ...
  proof-
    ...
    ultimately show ?thesis 
      by (metis cancel_ab_semigroup_add_class.diff_right_commute)
  qed
  moreover have "m \<ge> 1" using assms(3) by simp
  moreover have "j div 2^(m-k-1) < 2^m" using assms(2) by (meson div_le_dividend le_less_trans) 
  ultimately show ?thesis using bin_rep_odd by (metis diff_diff_left)
qed

lemma decomp_unit_vec_one_right: 
(* AB: I'm not sure the name is meaningful, since one is an abbreviation.
Maybe ket_of_unit_tensor_ket_of_1 ? Or a bit shorter ket_unit_tensor_ket_one. 
Given our abbreviations in Deutsch.thy we used the prefixes/suffixes ket_one and ket_zero.
Might be better to stick to our chosen conventions, except if we think in retrospect that it's
...
proof
  ...
  next
    ...
    then have "|unit_vec (2^(k-1)) (m div 2)\<rangle> $$ (i div 2, j div 1) = 1" using a0 a1 ket_vec_def by simp
...
odd_two_times_div_two_nat plus_1_eq_Suc)
      ...
      then have "( |unit_vec (2^(k-1)) (m div 2)\<rangle> \<Otimes> one) $$ (i,j) = 0" using f1 f0 by simp
      then show "( |unit_vec (2^k) m\<rangle> )$$ (i,j) = ( |unit_vec (2^(k-1)) (m div 2)\<rangle> \<Otimes> one) $$ (i,j)"
        using assms(2) f0 a2 by (metis index_unit_vec(1) index_unit_vec(3) ket_vec_index)
    next
      assume "i div 2 \<noteq> m div 2"
...
next
  ...
  proof
    ...
    next
      ...
      moreover have "(j\<Otimes> 1 1 m j) = zero" using to_tensor_prod_def a1 by simp
      ultimately show "(j\<Otimes> 1 1 m j) = |unit_vec (2^1) (j div 2^(m-1))\<rangle>" by simp
    next
      ...
      moreover have "(j\<Otimes> 1 1 m j) = one" using to_tensor_prod_def a1 by simp
      ultimately show "(j\<Otimes> 1 1 m j) = |unit_vec (2^1) (j div 2^(m-1))\<rangle>" by simp
    qed
  qed
next
...
lemma as_unit: (* AB: a better name maybe ? What about ket_unit_vec_to_tensor_prod ?
Maybe ket_unit_to_tensor_prod for something a little bit shorter ? 
Of course, in that case aux_as_unit should become aux_ket_unit_vec_to_tensor_prod. *)
  assumes "j < 2^m" and "m \<ge> 1"
  shows "(j\<Otimes> 1 m m j) = |unit_vec (2^m) j\<rangle>" 
  using aux_as_unit assms by simp


subsection \<open>The Controlled Phase Gate CR\<close>
(* AB: you should have a look at the rules for capitalizing titles. It would also be useful 
...
(* AB: maybe rename with bin_frac ? When there are well known abbreviations and LaTeX commands 
, like \frac for instance, take advantage of those to shorten names. 
Why does your notation "bf _ _ " take only 2 arguments instead of 4 ? 
With something short as bin_frac, it's not even clear that you need an abbreviation anymore *)
...
(* AB: is qr a mere abbreviation or a definition ? 
If it's a definition can we find a better name ? *)
...
lemma adjoint_of_controlled_phase_shift: (* AB: idem *)
  fixes k
  defines "CR_adjoint \<equiv> Matrix.mat 4 4 (\<lambda>(i,j). if i = j then (if i = 3 then (exp (2*pi*-\<i>*1/2^k)) else 1) else 0)"
(* AB: CR_dagger to be coherent ? Adjoint and dagger are the same thing, but we have never used
the terminalogy adjoint so far. *)
...
lemma controlled_phase_shift_is_gate: (* AB: maybe CR_is_gate *)
  shows "gate 2 (CR k)"
proof
  show "dim_row (CR k) = 2\<^sup>2" using controlled_phase_shift_def by simp
next
  ...
  proof-
    ...
    next
      ...
      moreover have "(CR k)\<^sup>\<dagger> = Matrix.mat 4 4 (\<lambda>(i,j). if i = j then (if i = 3 then (exp (2*pi*-\<i>*1/2^k)) else 1) else 0)"
        using adjoint_of_controlled_phase_shift by simp     
...
proof-
  ...
  proof-
    ...
    moreover have "(bf (s-1) (r-1-1) m jd)+((bin_rep m jd)!(r-1))/2^(r-s+1) 
      ...
      using binary_fraction_def by simp
  ...
  ultimately show "(exp (complex_of_real (2*pi)*\<i>*(bf (s-1) (r-1-1) m jd))) * (exp (2*pi*\<i>*((bin_rep m jd)!(r-1))/2^(r-s+1)))
                 = (exp (complex_of_real (2*pi)*\<i>*(bf (s-1) (r-1) m jd)))" by simp
qed
...
proof
  ...
  then have f0: "i < 4 \<and> j = 0" using ket_vec_def qr_def by simp
  then have "((CR (r-s+1)) * ((qr s (r-1) m jd) \<Otimes> zero)) $$ (i,j) 
           = (\<Sum>k\<in>{0,1,2,3}. ((CR (r-s+1)) $$ (i,k)) * (((qr s (r-1) m jd) \<Otimes> zero) $$ (k,j)))" 
    using f0 ket_vec_def qr_def set_4 atLeast0LessThan controlled_phase_shift_def by simp
  then have f1: "((CR (r-s+1)) * ((qr s (r-1) m jd) \<Otimes> zero)) $$ (i,j) 
    ...
    using f0 ket_vec_def qr_def by simp
  ...
  moreover have "i=0 \<longrightarrow> ((qr s r m jd) \<Otimes> zero) $$ (i,j) = 1/sqrt(2)" 
    using qr_def ket_vec_def f0 by simp
...
next
  show "dim_col ((CR (r-s+1)) * ((qr s (r-1) m jd) \<Otimes> zero)) = dim_col ((qr s r m jd) \<Otimes> zero)"
    using controlled_phase_shift_def qr_def by simp
qed
...
proof
  ...
  proof-
   ...
   moreover have "exp (complex_of_real (2*pi)*\<i>*1/2^(r-s+1)*(bin_rep m jd)!(r-1)) 
                 = exp (complex_of_real (2*pi)*\<i>*1/2^(r-s+1)) " using assms by simp
   moreover have "(exp (complex_of_real (2*pi)*\<i>*(bf (s-1) (r-1-1) m jd))) * (exp (2*pi*\<i>*((bin_rep m jd)!(r-1))/2^(r-s+1)))
...
next
  show "dim_col ((CR (r-s+1)) * ((qr s (r-1) m jd) \<Otimes> one)) = dim_col ((qr s r m jd) \<Otimes> one)"
    using ket_vec_def controlled_phase_shift_def qr_def by simp
qed

(* AB: I will stop here for now. Please note that often tactics, in particular auto, can
be replaced by mere simp. Above I made the substitutions, but I'm sure one can do the same 
in many places in the rest of the formalization. *)
